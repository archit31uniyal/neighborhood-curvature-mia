...
def get_minK(text):
    assert args.openai_model is None, "get_minK not implemented for OpenAI models"
...
def generate_data(dataset, key, args, train=True):
    ...
    if type(dataset) == str:
        if dataset in custom_datasets.DATASETS:
            ...
            data = datasets.load_dataset('wikitext', 'wikitext-2-v1', cache_dir=cache_dir, ignore_verifications=True)[data_split][key]
        elif dataset == 'imdbhface':
            data = datasets.load_dataset('imdb', split=data_split, cache_dir=cache_dir)[key]
            data = data[:5000]
...
def load_base_model_and_tokenizer(name):
    ...
    if args.openai_model is None:
        ...
        if 'checkpoint' in name:
            ...
            base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, torch_dtype=torch.bfloat16, trust_remote_code = True, cache_dir=cache_dir)
    else:
        base_model = None
    ...
    if not args.skip_baselines:
        ...
        if args.openai_model is None:
            ...
            if args.ref_model is not None:
                ...
                with open(os.path.join(SAVE_FOLDER, f"min_K_threshold_results.json"), "w") as f:
